const eskuls = [
  {
    id: 1,

    title: "Pramuka",

    description:
      "Ekstrakurikuler Pramuka di SMPN 4 Gunung Putri bertujuan membentuk karakter siswa yang disiplin, mandiri, bertanggung jawab, dan memiliki jiwa kepemimpinan melalui kegiatan kepanduan, kerja sama tim, dan latihan rutin yang edukatif.",

    image: "/images/Pramuka.jpg",

    gallery: [
      "/images/pramuka1.jpg",
      "/images/pramuka2.jpg",
      "/images/pramuka3.jpg",
      "/images/pramuka4.jpg",
      "/images/pramuka5.jpg",
    ],

    video: "/videos/pramuka.mp4",

    pembinaPutra: "Yusup Ansori, S.Pd.",
    pembinaPutri: "Putri Oktarina, S.Pd.",

    category: "WAJIB",
  },


  {
    id: 2,

    title: "PMR",

    description:
      "Belajar hidup sehat, pertolongan pertama, dan meningkatkan kepedulian sosial antar siswa.",

    image: "/images/pmr.jpg",

    gallery: [
      "/images/pmr1.jpg",
      "/images/pmr2.jpg",
      "/images/pmr3.jpg",
      "/images/pmr4.jpg",
    ],

    video: "/videos/pmr.mp4",

    pembina: "Dela Rizkia, S.Pd.",

    category: "SOSIAL",
  },

  {
    id: 3,

    title: "Seni Tari",

    description:
      "Melestarikan budaya melalui seni tari tradisional maupun modern dengan kreativitas siswa.",

    image: "/images/tari.jpg",

    gallery: [
      "/images/tari1.jpg",
      "/images/tari2.jpg",
      "/images/tari3.jpg",
      "/images/tari4.jpg",
      "/images/tari5.jpg",
      "/images/tari6.jpg",
    ],

     video: [
      "/videos/tari.mp4",
      "/videos/tari1.mp4",
     ],
     pembina: "Puja Okzalia, S.Pd.",
     category: "SENI",
  },

  {
    id: 4,

    title: "Paduan Suara",

    description:
      "Mengembangkan bakat vokal, harmonisasi suara, dan kekompakan dalam bernyanyi.",

    image: "/images/padus.jpg",

    gallery: [
      "/images/padus1.jpg",
      "/images/padus2.jpeg",
      "/images/padus3.jpg",
      "/images/padus4.jpg",
    ],

    video: "/videos/padus.mp4",

    pembina: "Andini Putri Nadira",
    category: "MUSIK",
  },

  {
    id: 5,

    title: "Marching Band",

    description:
      "Mengembangkan kemampuan bermusik, kekompakan, dan kedisiplinan dalam penampilan marching band.",

    image: "/images/marchingband.jpg",

    gallery: [
      "/images/marching1.jpg",
      "/images/marching2.jpg",
      "/images/marching3.jpg",
      "/images/marching4.jpg",
    ],

     video: "/videos/marchingband.mp4",
     pembina: "Findya Nurfajrin, S.Pd.",

    category: "MUSIK",
  },

  {
    id: 6,

    title: "Futsal",

    description:
      "Mengasah keterampilan bermain bola, strategi permainan, dan kerja sama tim.",

    image: "/images/futsal.jpg",

    gallery: [
      "/images/futsal1.jpg",
      "/images/futsal2.jpg",
      "/images/futsal3.jpg",
      "/images/futsal4.jpg",
      "/images/futsal5.jpg",
      "/images/futsal6.jpg",
    ],

    video: [
     "/videos/futsal.mp4",
    "/videos/futsal1.mp4",  
    ],
    pembina: "M. Fudhail Al Ubad, S.Pd.",
    category: "OLAHRAGA",
  },

  {
    id: 7,

    title: "Voli",

    description:
      "Melatih kemampuan bermain bola voli, kerja sama tim, dan sportivitas siswa.",

    image: "/images/volii.jpg",

    gallery: [
      "/images/voli1.jpg",
      "/images/voli2.jpg",
      "/images/voli3.jpg",
      "/images/voli4.jpg",
      "/images/voli5.jpg",                            
    ],

    video: "/videos/voli.mp4",
    pembina: "Karina Nur Azzahra, S.Pd.",
    category: "OLAHRAGA",
  },

  {
    id: 8,

    title: "Bulu Tangkis",

    description:
      "Mengasah kelincahan, fokus, dan teknik bermain bulu tangkis secara sportif.",

    image: "/images/bulutangkis.jpg",

    gallery: [
      "/images/bulutangkis1.jpg",
      "/images/bulutangkis2.jpg",
      "/images/bulutangkis3.jpg",
      "/images/bulutangkis4.jpg",
    ],

    video: "/videos/bulutangkis.mp4",
    pembina: "Renca Adhi Nugraha, S.Pd.",
    category: "OLAHRAGA",
  },

  {
    id: 9,

    title: "Pancak Silat",

    description:
      "Melatih seni bela diri tradisional, kedisiplinan, dan rasa percaya diri siswa.",

    image: "/images/pencaksilat.jpg",

    gallery: [
      "/images/silat1.jpg",
      "/images/silat2.jpg",
      "/images/silat3.jpg",
      "/images/silat4.jpg",
    ],

    video: "/videos/pencaksilat.mp4",
    pembina: "Muhamad Yoga Kautsar, S.Pd.",
    category: "SENI BELA DIRI",
  },
];

module.exports = eskuls;