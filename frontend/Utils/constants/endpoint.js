// Membaut variable API_KEY
const API_KEY = import.meta.env.VITE_API_KEY;

// Membuat variable BASE_URL
const BASE_URL = "https://api.themoviedb.org/3";

const params = `?api_key=${API_KEY}&append_to_response=videos`;

const ENDPOINTS = {POPULAR: `${BASE_URL}/movie/popular?api_key=${API_KEY}`,
    TOP_RATED: `${BASE_URL}/movie/top_rated?api_key=${API_KEY}`,
    NOW_PLAYING: `${BASE_URL}/movie/now_playing?api_key=${API_KEY}`,
    HERO: `${BASE_URL}/trending/movie/day?api_key=${API_KEY}`,
    DETAIL: (id) => `${BASE_URL}/movie/${id}?api_key=${API_KEY}&append_to_response=videos`,
    RECOMMENDATIONS: (id) => `${BASE_URL}/movie/${id}/recommendations?api_key=${API_KEY}`,
    TRENDING: `${BASE_URL}/trending/movie/day?api_key=${API_KEY}`,
    GET_RECOMMENDATIONS: (id) => `${BASE_URL}/movie/${id}/recommendations${params}`,
    FETCH_MOVIE_DETAILS: (movieId) => `${BASE_URL}/movie/${movieId}${params}`,
};

export default ENDPOINTS;